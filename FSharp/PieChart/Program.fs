...
let drawLabel (gr : Graphics, title, startAngle, angle) = 
    ...
    let size = gr.MeasureString(title, font)
    let rc = new PointF(float32 (x) - size.Width / 2.0f, float32 (y) - size.Height / 2.0f)
    gr.DrawString(title, font, Brushes.Black, new RectangleF(rc, size))
...
let saveDrawing (e) = 
    ...
    if (dlg.ShowDialog() = DialogResult.OK) then boxChart.Image.Save(dlg.FileName)
